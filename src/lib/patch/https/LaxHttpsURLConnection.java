...
public class LaxHttpsURLConnection
        extends javax.net.ssl.HttpsURLConnection {

    // NOTE: made protected for plugin so that subclass can set it.
    protected LaxDelegateHttpsURLConnection delegate;

    // For both copies of the file, uncomment one line and comment the other
    public LaxHttpsURLConnection(URL u, Handler handler) throws Exception {
        this(u, null, handler);
    }

    public LaxHttpsURLConnection(URL u, String hostname, int port, Handler handler) throws Exception {
        super(u);
        delegate = new LaxDelegateHttpsURLConnection(url, new Proxy(Proxy.Type.HTTP, InetSocketAddress.createUnresolved(hostname, port)), handler, this);
    }

    // For both copies of the file, uncomment one line and comment the other
    public LaxHttpsURLConnection(URL u, Proxy p, Handler handler) throws Exception {
        super(u);
        delegate = new LaxDelegateHttpsURLConnection(url, p, handler, this);
    }

    // NOTE: introduced for plugin
    // subclass needs to overwrite this to set delegate to
    // the appropriate delegatee
    protected LaxHttpsURLConnection(URL u) throws Exception {
        super(u);
    }
